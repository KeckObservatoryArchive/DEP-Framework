...
from dep_pipeline.core.common import update_koatpx
import os
import shutil
...
class dep_locate(BasePrimitive):
    ...
    def __init__(self, action, context):
        ...
        self.ancDir   = self.context.instrObj.dirs['anc']
        self.stageDir = self.context.instrObj.dirs['stage']


    def _perform(self):
        #Find sdata dirs list. Return if none found.
        if ('SEARCH_DIR' in self.config.dep.properties):
            self.logger.info("Using search directory defined in SEARCH_DIR: %s" % self.config.dep.SEARCH_DIR)
            useDirs = [self.config.dep.SEARCH_DIR]
        else:
            useDirs = self.context.instrObj.get_dir_list()
            self.logger.inf("Using get_dir_list to get SEACH_DIR: %s" % useDirs)

        if len(useDirs) == 0:
            self.logger.error('dep_locate: Did not find any directories to search!')
            return


        # Create files to store the valid FITS files
        presort1File = ''.join((self.stageDir, '/dep_locate', self.instr, 'pre1.text'))
        presort2File = ''.join((self.stageDir, '/dep_locate', self.instr, 'pre2.text'))
        locateFile  = ''.join((self.stageDir, '/dep_locate', self.instr, '.txt'))


        # Find the files in the last 24 hours
        self.logger.info('Looking for FITS files in {}'.format(useDirs))
        modtimeOverride = int(self.config.dep.MODTIME_OVERRIDE) if 'MODTIME_OVERRIDE' in self.config.dep.properties else 0
        filePaths = find_24hr_fits(useDirs, self.context.instrObj.utDate, self.context.instrObj.endTime, modtimeOverride)

        # write filepaths to outfile
        with open(presort1File, 'w') as f:
            for path in filePaths:
                f.write(path + '\n')
        self.logger.info('Copied fits file names from {} to {}'.format(useDirs, presort1File))
            ...
            with open(presort1File, 'r') as pre:
                ...
                for line in pre:
                    if ('.fits' in line
                        ...
                        newFile = ''.join((self.stageDir, line.strip()))
                        self.logger.info('copying file {} to {}'.format(line.strip(), newFile))
                        ...
                        if 'DEIMOS' in self.instr:
                            try:
                                ...
                                if fcs != '' and fcs not in fcsConfigs:
                                    ...
                                    if '/s/' not in fcs:
                                        fcs = '/s' + fcs
                                    newFile = ''.join((self.stageDir, fcs))
        ...
        isReprocess = int(self.config.dep.REPROCESS) if 'REPROCESS' in self.config.dep.properties else 0
        locateFile = self.stageDir + '/dep_locate' + self.instr + '.txt'
        dep_rawfiles(self.instr, self.utDate, presort2File, locateFile, self.ancDir, isReprocess, self.logger)

        # log completion with count
        num = sum(1 for line in open(locateFile, 'r'))
        self.logger.info('dep_locate: {} {} FITS files passed final checks.'.format(num, self.instr))

        # update koatpx
        if self.tpx:
            utcTimestamp = dt.utcnow().strftime("%Y%m%d %H:%M")
            update_koatpx(self.instr, self.utDate, 'files', str(num), self.logger)
            update_koatpx(self.instr, self.utDate, 'ondisk_stat', 'DONE', self.logger)
            update_koatpx(self.instr, self.utDate, 'ondisk_time', utcTimestamp, self.logger)
